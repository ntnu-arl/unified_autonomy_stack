...
def run_experiment(exp_id):
    ...
    subprocess.run(["make", "launch", "DOCKER_COMPOSE_FILE=docker-compose.uav_rl_only_eval.yml", "DOCKER_ARGS=-d"])
    ...
    if node.last_pose is None:
        ...
        node.destroy_node()
        rclpy.shutdown()
        subprocess.run(["make", "stop", "DOCKER_COMPOSE_FILE=docker-compose.uav_rl_only_eval.yml"])
