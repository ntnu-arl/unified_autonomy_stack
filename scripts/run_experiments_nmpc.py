...
class ExperimentLogger(Node):
    ...
    def pc_callback(self, msg):
        ...
        # print(f"pc min dist: {min_dist:.2f}")
        if min_dist < 0.3:
...
def run_experiment(exp_id):
    ...
    # subprocess.run(["make", "launch", "DOCKER_COMPOSE_FILE=docker-compose.uav_nmpc_unipilot_only_eval.yml", "DOCKER_ARGS=-d"])
    subprocess.run(["make", "launch", "DOCKER_COMPOSE_FILE=docker-compose.uav_nmpc_unipilot_only_eval.yml", "DOCKER_ARGS="])
    ...
    if node.last_pose is None:
        ...
        node.destroy_node()
        rclpy.shutdown()
        subprocess.run(["make", "stop", "DOCKER_COMPOSE_FILE=docker-compose.uav_nmpc_unipilot_only_eval.yml"])
        return False # Signal failure to start

    # Force daemon restart so it picks up ROS_DOMAIN_ID=205
    subprocess.run(["ros2", "daemon", "stop"], capture_output=True)
    subprocess.run(["ros2", "daemon", "start"], capture_output=True)
    time.sleep(2)

    print("Sim stabilized. Waiting for NMPC node /sdf_nmpc/sdfnmpc to be ready...")
    start_wait = time.time()
    nmpc_ready = False
    while time.time() - start_wait < 30:
        result = subprocess.run(["ros2", "node", "list"], capture_output=True, text=True)
        if '/sdf_nmpc/sdfnmpc' in result.stdout:
            nmpc_ready = True
            print("NMPC node /sdf_nmpc/sdfnmpc is ready.")
            break
    ...
    # 3. Enable NMPC controller via service call, then publish path
    # print("Calling /sdf_nmpc/set_flag service to enable NMPC controller...")
    # set_flag_client = node.create_client(SetBool, '/sdf_nmpc/set_flag')
    # set_flag_req = SetBool.Request()
    # set_flag_req.data = True
    # set_flag_success = False
    # while not set_flag_success:
    #     set_flag_client.wait_for_service(timeout_sec=5.0)
    #     future = set_flag_client.call_async(set_flag_req)
    #     rclpy.spin_until_future_complete(node, future, timeout_sec=5.0)
    #     if future.result() is not None and future.result().success:
    #         print("set_flag service returned success.")
    #         set_flag_success = True
    #     else:
    #         reason = future.result().message if future.result() is not None else "no response"
    #         print(f"set_flag returned false ({reason}), retrying in 5s...")
    #         time.sleep(5)
    ...
    os.makedirs('logs/nmpc/accel', exist_ok=True)
    with open(f'logs/nmpc/accel/exp_{exp_id}.csv', 'w', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=['t', 'ax', 'ay', 'az'])
        writer.writeheader()
        writer.writerows(node.accel_history)

    # Append to summary
    os.makedirs('logs/nmpc', exist_ok=True)
    summary_file = 'logs/nmpc/experiment_summary.csv'
    ...
    with open('logs/nmpc/experiment_summary.txt', 'a') as f:
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\nAll experiments complete! Summary saved to logs/nmpc/experiment_summary.csv")
